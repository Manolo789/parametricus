...
- [X] `distance(a, b)` — entre pontos, ponto→superfície (o SDF **é** a função distância: `solid.distance(p)` já responde isso de graça — vantagem da arquitetura), e mínima entre dois sólidos (otimização sobre os dois campos).
- [X] `angle(v1, v2)` / entre faces planares detectadas na malha.
